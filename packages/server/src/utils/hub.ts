export function parsePrompt(prompt: string): any[] {
    ...
    if (promptObj.kwargs.messages) {
        promptObj.kwargs.messages.forEach((message: any) => {
            const messageType = message.id.includes('SystemMessagePromptTemplate')
                ? 'systemMessagePrompt'
                ...
                ? 'aiMessagePrompt'
                : 'template'
            const messageTypeDisplay = message.id.includes('SystemMessagePromptTemplate')
                ? 'System Message'
                : message.id.includes('HumanMessagePromptTemplate')
                ? 'Human Message'
                : message.id.includes('AIMessagePromptTemplate')
                ? 'AI Message'
                : 'Message'
            const template = message.kwargs.prompt.kwargs.template
            response.push({
                type: messageType,
        ...
        const template = promptObj.kwargs.template
        response.push({
            type: 'template',
            typeDisplay: 'Prompt',
            template: template
